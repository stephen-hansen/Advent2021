...
# day 4 specific functions, not standard lib
fun makeBundle(item) {
	var tab
	tab : tab_alloc(2)
	tab_addr(.tab, 0) : .item
...
fun filterIfNotEmpty(lines) {
	var i, str, k, n, newtab
	k : 0
	n : tab_len(.lines)
	i : 0
	loop {
		until .i == .n
		str : tab_index(.lines, .i)
		if stringLen(.str) > 0 {
			k : .k + 1
		}
		i : .i + 1
...
fun makeBundles(lines) {
	var n, newtab, i, bundle
	n : tab_len(.lines)
	newtab : tab_alloc(.n)
	i : 0
	loop {
		until .i == .n
		bundle : makeBundle(tab_index(.lines, .i))
		tab_addr(.newtab, .i) : .bundle
		i : .i + 1
	}
	return .newtab
}
...
# Calc number of boards
	...
	loop {
		until .i == .n
		if isEmptyStr(tab_index(.lines, .i)) {
			nboards : .nboards + 1
		}
		i : .i + 1
	}
# Load boards
	boards : tab_alloc(.nboards)
	i : 2
	board : tab_alloc(5)
	bp : 0
	bp0 : 0
	loop {
		...
		if isEmptyStr(.line) == 1 {
			...
			numbers : filterIfNotEmpty(.numbers)
			numbers : stringsToNumbers(.numbers, 10)
			numbers : makeBundles(.numbers)
			tab_addr(.board, .bp0) : .numbers
			bp0 : .bp0 + 1
		}
		i : .i + 1
	}
	tab_addr(.boards, .bp) : .board
# Return boards
# Structure is i (board ID) -> j (row) -> k (col) -> 0 for value, 1 for boolean
	return .boards
}

fun winningBoard(board) {
	var i, j, n0, n1, row, cell, complete
# Rows
	i : 0
	n0 : tab_len(.board)
	loop {
		until .i == .n0
		row : tab_index(.board, .i)
		j : 0
		n1 : tab_len(.row)
		complete : 1
		loop {
			until .j == .n1
			cell : tab_index(.row, .j)
			if tab_index(.cell, 1) == 0 {
				complete : 0
			}
			j : .j + 1
		}
...
# Cols
	...
	loop {
		...
		loop {
			until .j == .n1
			cell : tab_index(tab_index(.board, .j), .i)
			if tab_index(.cell, 1) == 0 {
				complete : 0
			}
			j : .j + 1
...
fun scoreBoard(board) {
	var i, j, n0, n1, row, cell, sum
	i : 0
	sum : 0
	n0 : tab_len(.board)
	loop {
